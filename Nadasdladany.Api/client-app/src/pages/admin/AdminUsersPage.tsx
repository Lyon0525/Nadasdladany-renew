            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-primary/20 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
                    <div className="bg-white rounded-[32px] p-8 max-w-md w-full shadow-2xl border border-gray-100 relative animate-in zoom-in-95 duration-200">
                        <button onClick={() => setIsFormOpen(false)} className="absolute top-6 right-6 p-2 text-gray-400 hover:text-primary rounded-full hover:bg-gray-50 transition-colors cursor-pointer"><X size={18} /></button>
                        <h3 className="text-2xl font-serif font-bold text-primary mb-2">Új munkatárs regisztrációja</h3>
                        <p className="text-xs text-gray-400 mb-6">A megadott e-mail címmel és jelszóval a kolléga azonnal be tud majd lépni a teljes admin felületre.</p>

                        <form onSubmit={handleCreateUser} className="space-y-4">
                            <div>
                                <label className="block text-[11px] font-bold uppercase text-gray-400 mb-1.5">Hivatali e-mail cím</label>
                                <input type="email" required placeholder="nev@nadasdladany.hu" className="w-full bg-gray-50 border border-gray-200/60 p-3.5 rounded-xl outline-none focus:border-accent text-sm" value={email} onChange={e => setEmail(e.target.value)} />
                            </div>
                            <div>
                                <label className="block text-[11px] font-bold uppercase text-gray-400 mb-1.5">Kezdeti jelszó</label>
